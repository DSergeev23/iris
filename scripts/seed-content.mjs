...
try {
  ...
  for (const [sortOrder, record] of records.entries()) {
    ...
    const department = await db.department.create({ data: {
      ...
      scenario: { create: { title: "Провести по шагам", description: "Ответьте на несколько простых вопросов.", emergencyBody: "Позовите медсестру кнопкой вызова у кровати или обратитесь на пост.", status: PublicationStatus.DRAFT } },
    }, include: { scenario: true } });
    const steps = await Promise.all([
      ["Что нужно сейчас?", "Выберите самый подходящий вариант."],
      ["Следующий шаг", "Выполняйте рекомендации только с учётом назначений врача."],
    ...
    await db.scenarioAction.createMany({ data: [
      { stepId: steps[0].id, title: "Нужны рекомендации", body: "Хочу понять следующий шаг.", actionLabel: "Продолжить", kind: ScenarioActionKind.STEP, targetStepId: steps[1].id, sortOrder: 0 },
