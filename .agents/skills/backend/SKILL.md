...
name: backend-dev
description: |
  Backend architecture, API/service layer, database access, auth middleware, observability,
  background jobs, caching, file upload endpoints, real-time server features, and production hardening guide.
  TRIGGER when: scaffolding backend service, creating REST API, GraphQL/tRPC/gRPC backend,
  Express + React backend side, Next.js API, Node.js backend, Python backend, Go backend,
  designing service layers, implementing error handling, managing config/auth,
  database access, migrations, caching, background jobs, logging, health checks,
  CORS/security headers, file upload server endpoints, SSE/WebSocket server,
  hardening backend for production.
  DO NOT TRIGGER when: pure frontend UI work, pure CSS/styling,
  frontend-only API client implementation, frontend-only loading/error states,
  database schema only without backend implementation context.
license: MIT
metadata:
  category: backend
  version: "1.0.0"
...
**When this backend skill is triggered, you MUST follow this workflow before writing any code.**

### Step 0: Gather Requirements

Before scaffolding anything, ask the user to clarify (or infer from context):

1. **Stack**: Language/framework for backend and, if relevant, frontend consumer (e.g., Express + React, Django + Vue, Go + HTMX)
...
| API boundary | REST / GraphQL / tRPC / OpenAPI / gRPC | [Section 5](#5-api-client-patterns-medium) |
...
Use the appropriate backend checklist below. Ensure ALL checked items are implemented — do not skip any.
...
1. **Build check**: Ensure backend compiles without errors
   ```bash
...
2. **Start & smoke test**: Start the server, verify key endpoints return expected responses
   ...
   ```
3. **Integration check**: Verify frontend can connect to backend when a frontend exists (CORS, API base URL, auth flow)
4. **Real-time check** (if applicable): Open two browser tabs or two clients, verify changes sync
...
- **What was built**: List of implemented backend features and endpoints
- **How to run**: Exact commands to start backend and, if relevant, frontend
...
**USE this skill when:**
- Building a backend application, API, backend service, or backend side of a full-stack application
...
- Hardening backend for production
- Setting up auth middleware, file upload endpoints, real-time server features, CORS, health checks, or security headers

**NOT for:**
- Pure frontend/UI concerns
- Pure CSS/styling
- Frontend-only API client implementation
...
## Quick Start — Frontend-Backend Integration Checklist (Backend Contract Responsibilities)
...
| Need to… | Jump to |
|----------|---------|
| Organize backend project folders | [1. Project Structure](#1-project-structure--layering-critical) |
...
| Choose/define API boundary for frontend consumers | [5. API Client Patterns](#5-api-client-patterns-medium) |
...
| Define API error contract for frontend | [12. Cross-Boundary Error Handling](#12-cross-boundary-error-handling-medium) |
...
## 5. API Client Patterns (MEDIUM) — Backend Contract Awareness
...
## 12. Cross-Boundary Error Handling (MEDIUM) — Backend Error Contract
